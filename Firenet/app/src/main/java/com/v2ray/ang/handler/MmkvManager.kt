...
/**
    ...
    private const val KEY_LAST_STATUS = "last_status_response"

    fun saveLastStatus(resp: StatusResponse) {
        val json = JsonUtil.toJson(resp)
        MMKV.defaultMMKV().encode(KEY_LAST_STATUS, json)
    }

    fun loadLastStatus(): StatusResponse? {
        val json = MMKV.defaultMMKV().decodeString(KEY_LAST_STATUS, null) ?: return null
        return try {
            JsonUtil.fromJson(json, StatusResponse::class.java)
        } catch (e: Exception) {
            null
        }
    }
